...
const navItems: NavItem[] = [
  ...
  { href: "/mis_reservas", label: "Mis Reservas" },
];
  ...
  return (
    <header className="sticky top-0 z-50 flex h-20 px-8 md:px-20 lg:px-36 justify-between items-center self-stretch bg-white shadow-sm shadow-primary">
      <div className="flex items-center gap-10 md:gap-16">
        ...
        <div>
          <nav className="hidden md:flex gap-10 justify-center items-center text-black text-[1.25rem] font-semibold leading-normal font-barlow">
            {navItems.map((item) => {
              const isActive =
                pathname === item.href ||
              ...
              return (
                <Link
                  ...
                  className={`transition-colors ${
                    isActive
                      ? "text-primary bg-secondary px-5 py-1 rounded-[9999px]"
                      : "hover:text-primary hover:bg-secondary/50 px-5 py-1 rounded-[9999px]"
                  }`}
                >
                  {item.label}
                </Link>
          ...
          {showUserMenu && (
            <div className="absolute right-0 mt-2 w-56 bg-white rounded-lg shadow-lg border border-gray-200 py-2 z-50">
              <div className="px-4 py-3 border-b border-gray-200">
                <p className="text-sm text-gray-500">Bienvenid@,</p>
